...
export async function renderOverview(container: HTMLElement) {
  await withView(
    ...
    ([memories, skills, agents, tasks, commands, workflows, settings]) => `
      <h2>Visión general</h2>
      <p class="lead">Estado en vivo de <code>~/.claude</code> — todo lo de acá se lee directo de tus archivos reales.</p>
      <div class="grid">
        <div class="card inert"><h3>🧩 Memoria <span class="count">(${memories.length})</span></h3><p>Entradas activas en tu memoria del chat global.</p></div>
        <div class="card inert"><h3>🛠️ Skills <span class="count">(${skills.length})</span></h3><p>Skills instaladas en <code>~/.claude/skills</code>.</p></div>
        <div class="card inert"><h3>🤖 Agentes <span class="count">(${agents.length})</span></h3><p>Agentes custom en <code>~/.claude/agents</code>.</p></div>
